        ...
        return Preconditions.check(new UsesType<>("java.math.BigDecimal", false), new JavaIsoVisitor<ExecutionContext>() {
            ...
            public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, ExecutionContext ctx) {
                ...
                if (BIG_DECIMAL_DIVIDE.matches(m) &&
                    ...
                    m = twoArgDivide.apply(updateCursor(m), m.getCoordinates().replaceArguments(), m.getArguments().getFirst(), roundingModeEnum);
                    ...
                    m = twoArgScale.apply(updateCursor(m), m.getCoordinates().replaceArguments(), m.getArguments().getFirst(), roundingModeEnum);
                    maybeAddImport("java.math.RoundingMode");
                    ...
                    m = threeArg.apply(updateCursor(m), m.getCoordinates().replaceArguments(), m.getArguments().getFirst(), m.getArguments().get(1), roundingModeEnum);
                    maybeAddImport("java.math.RoundingMode");
                }
                return m;
            }
            ...
            private boolean isConvertibleBigDecimalConstant(J elem) {
                if (elem instanceof J.Literal) {
                    return true;
                } else if (elem instanceof J.FieldAccess fa && fa.getTarget().getType() instanceof JavaType.FullyQualified) {
            ...
            private String getTemplateText(J elem) {
                ...
                if (elem instanceof J.FieldAccess fa && fa.getTarget().getType() instanceof JavaType.FullyQualified) {
                ...
                } else if (elem instanceof J.Literal literal) {
                    roundingName = literal.getValueSource();
                }
